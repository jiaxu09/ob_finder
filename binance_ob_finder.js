...
const RUNTIME_CONFIG = {
  ...
  TIMEFRAME_WINDOWS: {
    ...
    "5m": 20, // 20分钟（4次检测机会）
    "15m": 30, // 30分钟（6次检测机会）
    "1h": 90, // 1.5小时（18次检测机会） ← 从150改为90
    "4h": 300, // 5小时（60次检测机会） ← 从600改为300
    "1d": 1500, // 25小时（300次检测机会） ← 从3000改为1500
  },
  ...
  STORAGE_CONFIG: {
    ...
    CLEANUP_DAYS: 30,
    AUTO_CREATE_BUCKET: true, // 🆕 自动创建Bucket
  },
  ...
  DB_CONFIG: {
    ...
    DAYS_LOOKBACK: 30,
    SAVE_FULL_DATA: true,
    USE_DB_FALLBACK: true, // 🆕 Storage失败时使用Database
  },
};
...
function analyzeBreakoutCandlePattern(breakoutCandle, obType) {
  ...
  return {
    ...
    recommendation,
    description: generateCandleDescription(
      bodyPercent,
      upperWickPercent,
      lowerWickPercent,
      isBullish
...
function generateCandleDescription(
  bodyPercent,
  upperWickPercent,
  lowerWickPercent,
  isBullish
...
function formatOBDetails(ob, index, symbol, timeframe) {
  ...
  return `
╔═══════════════════════════════════════════════════════════════════════════════
║ OB #${index + 1} - ${symbol} ${timeframe} - ${
    ob.type === "Support" ? "🟢 BULLISH SUPPORT" : "🔴 BEARISH RESISTANCE"
  }
╠═══════════════════════════════════════════════════════════════════════════════
║ 📍 价格区间
...
║
║ 🕯️ 突破K线形态
║   ├─ 形态: ${bp.candleEmoji} ${bp.candleType}
║   ├─ 强度: ${bp.breakoutEmoji} ${bp.breakoutStrength} (${
    bp.strengthScore
  }/100)
...
function logAllOBs(allZonesData, context) {
  context.log("\n" + "█".repeat(80));
  context.log("📊 所有检测到的 ORDER BLOCKS");
  context.log("█".repeat(80));

  let total = 0;
  for (const { symbol, timeframe, zones } of allZonesData) {
    const count = zones.bullishOBs.length + zones.bearishOBs.length;
    total += count;
    if (count > 0) {
      context.log(
        `${symbol} ${timeframe}: 🟢${zones.bullishOBs.length} 🔴${zones.bearishOBs.length}`
      );
    }
  }
  context.log(`总计: ${total} 个OBs\n`);
}

// ============================================================================
// --- Order Block 技术指标计算（保持不变，代码省略）---
// ============================================================================
...
function calculateAtrEma(klines, period = 10) {
  ...
  const trs = klines.map((k, i) =>
    calculateTrueRange(k, i > 0 ? klines[i - 1] : null)
  );
  const alpha = 1 / period;
  let atr =
    trs.slice(1, period + 1).reduce((sum, val) => sum + val, 0) / period;
...
function findOrderBlocksPineScriptLogic(
  ...
  for (let barIndex = swingLength; barIndex < klines.length; barIndex++) {
    ...
    if (
      ...
      const balancePercent = calculateBalancePercentage(
        obHighVolume,
        obLowVolume
      );
      if (
        balancePercent < minBalancePercent ||
...
function detectPotentialNewZones(allZonesData, context) {
  ...
  for (const { symbol, timeframe, zones } of allZonesData) {
    ...
    if (recentZones.length > 0) {
      context.log(
        `  🆕 ${symbol} ${timeframe}: ${recentZones.length} 个潜在新zones (窗口: ${windowMinutes}分钟)`
      );
 ...
 * 🆕 确保Storage Bucket存在
 */
async function ensureStorageBucketExists(storage, context) {
  ...
  try {
    ...
    if (RUNTIME_CONFIG.STORAGE_CONFIG.AUTO_CREATE_BUCKET) {
      try {
        // 尝试创建bucket
        await storage.createBucket(
          bucketId,
          bucketId,
          ['read("any")', 'write("any")'], // 权限设置
          false, // file security
          true, // enabled
          5242880, // 5MB max file size
          ["json"] // allowed extensions
        );
        context.log(`✅ 自动创建 Storage Bucket "${bucketId}"`);
        return true;
      } catch (createError) {
        context.error(`❌ 无法创建 Storage Bucket: ${createError.message}`);
        return false;
      }
    } else {
      context.error(`❌ Storage Bucket "${bucketId}" 不存在且未启用自动创建`);
      return false;
    }
  }
}
...
async function loadZonesFromStorage(storage, context) {
  try {
    ...
    context.log(
      `✅ 从Storage加载: ${data.zones.length} 条记录 (更新于: ${data.lastUpdated})`
    );
    return new Set(data.zones);
  } catch (e) {
    if (e.message && e.message.includes("not found")) {
      context.log("⚠️ Storage文件不存在，返回空Set");
      return new Set();
    }
...
async function cleanupStorageZones(zones, context) {
  const cutoffTime =
    Date.now() -
    RUNTIME_CONFIG.STORAGE_CONFIG.CLEANUP_DAYS * 24 * 60 * 60 * 1000;
  ...
  if (removed > 0) {
    context.log(
      `🗑️ 清理过期记录: ${removed} 条 (保留: ${cleanedZones.size} 条)`
    );
  }
...
// ============================================================================
// --- 🔑 Database备份系统（作为Storage失败的后备）---
// ============================================================================

/**
 * 🆕 从Database加载zones（Storage失败时的后备方案）
 */
...
/**
 * 🆕 保存到Database（立即保存，不异步）
 */
async function saveZonesToDatabase(
  databases,
  DB_ID,
  COLLECTION_ID,
  newZoneIdentifiers,
  ...
  try {
    const promises = newZoneIdentifiers.map((zoneId) =>
      databases
        .createDocument(DB_ID, COLLECTION_ID, ID.unique(), {
...
function shouldSyncToDatabase() {
  ...
  return (
    hour === RUNTIME_CONFIG.DB_CONFIG.SYNC_HOUR_UTC &&
    minute < RUNTIME_CONFIG.DB_CONFIG.SYNC_WINDOW_MINUTES
  );
}
...
function generateNotificationMessage(symbol, timeframe, zone, CONFIG) {
  ...
  const message =
    ...
    `*交易对:* ${symbol}\n` +
    `*周期:* ${timeframe}\n` +
    `*类型:* ${zone.type === "Support" ? "🟢 看涨支撑" : "🔴 看跌阻力"}\n` +
    `*状态:* ${status}\n` +
    `*价格:* ${zone.bottom.toFixed(
      zone.bottom > 100 ? 2 : 4
    )} - ${zone.top.toFixed(zone.top > 100 ? 2 : 4)}\n\n` +
    `*📊 成交量:* ${zone.volumeRatio}x (${
      parseFloat(zone.volumeRatio) >= 1.2 ? "✅" : "❌"
    })\n` +
    `*⚖️ 平衡度:* ${zone.balancePercent}% ${zone.balanceQuality}\n` +
    `*🕯️ K线强度:* ${bp.breakoutEmoji} ${bp.breakoutStrength} (${bp.strengthScore}/100)\n` +
    `*⏰ 时段:* ${sessionInfo.emoji} ${sessionInfo.session}${reliabilityWarning}\n` +
    `*💡 建议:* ${bp.recommendation}${patternWarning}`;

  const subject = `🔔 ${symbol} ${timeframe} 新${zone.type} [${bp.breakoutStrength}] [${sessionInfo.session}]`;
...
module.exports = async (context) => {
  ...
  context.log("🚀 Function started (v4.2 - 修正重复通知问题)");
  context.log(`⏰ ${new Date().toISOString()}`);
  context.log(
    `🔄 执行频率: 每 ${RUNTIME_CONFIG.EXECUTION_INTERVAL_MINUTES} 分钟\n`
  );
  ...
  // 🔑 确保Storage Bucket存在
  const bucketExists = await ensureStorageBucketExists(storage, context);

  // ============================================================================
  // 步骤1：分析OB
  // ============================================================================

  context.log("📊 Step 1: 分析交易对...\n");
  const allZonesData = [];
  ...
  for (const symbol of CONFIG.SYMBOLS) {
    for (const tf of CONFIG.TIMEZONES) {
      ...
      context.log(
        `${symbol} ${tf}: 🟢${result.bullishOBs.length} 🔴${result.bearishOBs.length}`
      );
      allZonesData.push({ symbol, timeframe: tf, zones: result });
    }
  }
  ...
  if (bucketExists) {
    ...
    if (previousZones.size === 0 && RUNTIME_CONFIG.DB_CONFIG.USE_DB_FALLBACK) {
      ...
      previousZones = await loadZonesFromDatabase(
        databases,
        DB_ID,
        COLLECTION_ID,
        context
  ...
  for (const potentialZone of potentialNewZones) {
    if (!previousZones.has(potentialZone.identifier)) {
      context.log(`  ✅ 新zone: ${potentialZone.identifier}`);
      ...
      context.log(`  ⏭️ 已存在: ${potentialZone.identifier}`);
    }
  }
  ...
  if (confirmedNewZones.length > 0) {
    context.log(`\n💾 Step 5: 保存 ${confirmedNewZones.length} 个新zones...`);

    previousZones = await cleanupStorageZones(previousZones, context);

    // 🔑 尝试保存到Storage，如果失败则保存到Database
    let saved = false;
    ...
    if (!saved || useDatabase) {
      ...
      const count = await saveZonesToDatabase(
        databases,
        DB_ID,
        COLLECTION_ID,
        confirmedNewZones,
  ...
  context.log("\n" + "=".repeat(80));
  context.log(`🎉 完成! 用时: ${executionTime}秒`);
  context.log(
    `💾 Storage: ${storageReads}读 ${storageWrites}写 | 📊 DB: ${databaseReads}读 ${databaseWrites}写`
  );
  context.log(`🆕 新zones: ${allNewNotifications.length}`);
  context.log("=".repeat(80) + "\n");
